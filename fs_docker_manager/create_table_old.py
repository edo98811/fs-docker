...
class Table():
  ...
  def add_processing_info(self, search_path_reconall: str, search_path_samseg: str, search_path_data: str) -> None:
    ...
    for index, row in self.table.iterrows():
      destination_folder = os.path.join(search_path_data, f"{row['acquisition']}", "flair_reg.nii")
      # print(os.path.exists(destination_folder))
      # print(destination_folder)
      if os.path.exists(destination_folder):
        # print("prepared")
        self.table.at[index, "samseg"] = "Prepared"
        # print(self.table.at[index, "samseg"])
    ...
    for index, row in self.table.iterrows():
      for _ , mri in enumerate(row["mris"]):
        # print(os.path.join(search_path_data, f"{row['acquisition']}", f"{mri}.nii.gz"))
        if os.path.isfile(os.path.join(search_path_data, f"{row['acquisition']}", f"{mri}.nii.gz")): 
          
          self.table.at[index, "converted"].append(True)
        else:
          # nifti_folder = os.path.join(search_path_data, f"{row['acquisition']}", f"{mri}.nii.gz")
